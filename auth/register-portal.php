...
$prefill_type = strtolower(trim($_GET['type'] ?? ''));
$allowed_prefill_types = ['applicant', 'employee'];
if (!in_array($prefill_type, $allowed_prefill_types, true)) {
    $prefill_type = '';
}
$source = strtolower(trim($_GET['source'] ?? ''));
$job_id = intval($_GET['job_id'] ?? 0);
$lock_to_prefill = $source === 'apply' && $prefill_type !== '';
?>
<!DOCTYPE html>
<html lang="en">
<head>
...
<body>
        ...
        <div class="form-panel">
            ...
            <div class="content-section">
                ...
                <div class="category-cards">
                    <!-- Applicant Card -->
                    <div class="category-card" onclick="selectCategory('applicant')" id="applicant-card" data-type="applicant">
                    ...
                    <!-- Employee Card -->
                    <div class="category-card" onclick="selectCategory('employee')" id="employee-card" data-type="employee">
                        <div class="category-icon">👤</div>
                        <div class="category-info">
    ...
    <script>
        ...
        const prefilledType = <?php echo json_encode($prefill_type); ?>;
        const lockToPrefill = <?php echo $lock_to_prefill ? 'true' : 'false'; ?>;
        const selectedJobId = <?php echo $job_id; ?>;

        function selectCategory(category) {
            // Don't allow selection of disabled categories
            if (category === 'both') return;

            // When coming from Apply flow, keep the preselected category fixed
            if (lockToPrefill && prefilledType && category !== prefilledType) {
                return;
            }
        ...
        function proceedToRegistration() {
            ...
            if (selectedCategory === 'applicant') {
                let target = 'register-applicant.php?terms_accepted=true';
                if (selectedJobId > 0) {
                    target += '&job_id=' + selectedJobId + '&source=apply';
                }
        ...
        // Auto-select and optionally lock type based on query params
        if (prefilledType) {
            selectCategory(prefilledType);
            if (lockToPrefill) {
                document.querySelectorAll('.category-card[data-type]').forEach((card) => {
                    const type = card.getAttribute('data-type');
                    if (type !== prefilledType) {
